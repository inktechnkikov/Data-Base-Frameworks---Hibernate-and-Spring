...
const kinveyAppKey = "kid_S1UeJN-U";
const kinveyAppSecret = "07ae69915c884c87947486496f4e6345";
...
function listBooks() {
    ...
    function loadBooksSuccess(books) {
        showInfo('The books has been loaded.');
        if (books.length == 0){
            $('#books').text ('No books.')
        } else {
            let booksTable = $('<table>')
                .append($('<tr>')
                    .append($('<th>').text('Ttile'))
...
function createBook() {
    ...
    function createBookSuccess(response){
        listBooks();
        showInfo('The book has been created.');
    }
}
